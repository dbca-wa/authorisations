...
- File: `VERSION`
- Required format: **`MAJOR.MINOR.PATCH` only** — no pre-release suffixes
- Example valid values (entries to put directly in the VERSION file):
  - `1.0.0`
  - `1.2.3`
  - `2.0.0`
- Example invalid values:
  ...
  - `1.0.0-uat` (do not add suffixes in the VERSION file; `-uat` suffix is generated by scripts)
  - `1.0.0-feature-name` (do not add suffixes in the VERSION file; `-feature-name` suffix is generated by scripts)
...
**Note:** Pre-release suffixes such as `-uat`, `-feature/branch-name`, and environment-specific tags are generated automatically by the build scripts (`scripts/get_image_tag.py` and `scripts/sync_version.py`) and applied to Docker images and kustomize overlays. Only the plain `MAJOR.MINOR.PATCH` version goes in the VERSION file.
