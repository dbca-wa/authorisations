...
@pytest.fixture
def reviewable_app(assessor_group, process_factory, questionnaire_factory, application_factory):
    """Return a submitted application in a process the assessor group can review."""
...
class TestApplicantTransitions:
    ...
    def test_cannot_bypass_triage(self, api_client, user, workflow_app):
        """Reject owner attempts to skip straight to technical assessment or decision."""
        api_client.force_authenticate(user=user)
        forbidden = [
    ...
    def test_withdraw_during_assessment(self, api_client, user, reviewable_app):
        """Allow owner to withdraw application during assessment phase."""
        api_client.force_authenticate(user=user)
        reviewable_app.owner = user  # Make user the owner
    ...
    def test_cannot_transition_from_terminal_state(self, api_client, user, workflow_app):
        """Reject attempts to transition from terminal states (APPROVED, REJECTED, etc.)."""
        api_client.force_authenticate(user=user)
        
        for terminal_status in [ApplicationStatus.APPROVED, ApplicationStatus.REJECTED, 
                               ApplicationStatus.DEFERRED, ApplicationStatus.WITHDRAWN]:
...
class TestAssessorTransitions:
    """Test transitions initiated by staff (Reviewers/Assessors)."""

    def test_staff_workflow_progression(self, api_client, assessor_user, reviewable_app):
        """Staff can move app through SUBMITTED -> UNDER_REVIEW -> UNDER_ASSESSMENT."""
        api_client.force_authenticate(user=assessor_user)
        
        # 1. Claim app
        response = api_client.patch(
            f"/api/assessment/{reviewable_app.key}",
    ...
    def test_return_to_draft_unlocks_editing(self, api_client, assessor_user, reviewable_app):
    ...
    def test_assessor_approve_decision(self, api_client, assessor_user, reviewable_app):
        """Allow assessor to approve an application under assessment."""
        api_client.force_authenticate(user=assessor_user)
        reviewable_app.status = ApplicationStatus.UNDER_ASSESSMENT
        reviewable_app.save()
    ...
    def test_assessor_approve_with_conditions_decision(self, api_client, assessor_user, reviewable_app):
        """Allow assessor to approve with conditions."""
        api_client.force_authenticate(user=assessor_user)
        reviewable_app.status = ApplicationStatus.UNDER_ASSESSMENT
        reviewable_app.save()
    ...
    def test_assessor_reject_decision(self, api_client, assessor_user, reviewable_app):
        """Allow assessor to reject an application."""
        api_client.force_authenticate(user=assessor_user)
        reviewable_app.status = ApplicationStatus.UNDER_ASSESSMENT
        reviewable_app.save()
    ...
    def test_assessor_defer_decision(self, api_client, assessor_user, reviewable_app):
        """Allow assessor to defer an application for later assessment."""
        api_client.force_authenticate(user=assessor_user)
        reviewable_app.status = ApplicationStatus.UNDER_ASSESSMENT
        reviewable_app.save()
    ...
    def test_return_to_draft_from_under_assessment(self, api_client, assessor_user, reviewable_app):
    ...
    def test_reviewer_cannot_set_applicant_only_transitions(self, api_client, assessor_user, reviewable_app):
        """Reject assessor attempts to set applicant-only statuses like DISCARDED."""
        api_client.force_authenticate(user=assessor_user)
    ...
    def test_cannot_skip_review_queue_progression(self, api_client, assessor_user, reviewable_app):
    ...
    def test_cannot_reverse_from_under_assessment_to_under_review(self, api_client, assessor_user, reviewable_app):
...
class TestWorkflowBusinessRules:
    ...
    def test_submitted_at_preservation(self, api_client, user, workflow_app, monkeypatch):
    ...
    def test_applicant_cannot_access_assessment_endpoint(self, api_client, user, workflow_app):
        """Reject applicant access to the assessment endpoint (staff-only)."""
        api_client.force_authenticate(user=user)
        workflow_app.status = ApplicationStatus.SUBMITTED
        workflow_app.save()
        
        response = api_client.patch(
            f"/api/assessment/{workflow_app.key}",
            {"status": ApplicationStatus.UNDER_REVIEW},
            format="json"
        )
    ...
    def test_reviewer_cannot_act_on_unrelated_application(self, api_client, assessor_user, workflow_app):
        """Reject staff access if application is not in a process they can review."""
        api_client.force_authenticate(user=assessor_user)
        # Application's process is not in assessor_user's group
        workflow_app.status = ApplicationStatus.SUBMITTED
        workflow_app.save()
